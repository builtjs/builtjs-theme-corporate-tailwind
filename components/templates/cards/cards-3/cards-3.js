    ...
    <section id="cards-3" className="template">
      <div className="max-w-screen-xl px-4 py-16 mx-auto md:py-24 xl:py-32">
        <div className="flex flex-col lg:flex-row">
          {items &&
            items.map((item, i) => (
              <div key={i} className="flex flex-col flex-1 p-8 my-6 bg-gray-100 rounded-lg lg:mx-5 lg:my-0 dark:bg-gray-700">
                <svg
                  className="w-12 h-12 p-3 mb-6 rounded-lg text-lightest bg-secondary-light"
                ...
                <h2 className="mb-4 text-2xl font-bold leading-none">{item.heading}</h2>
                <p>{item.blurb}</p>
                {item.buttonLinks && (
