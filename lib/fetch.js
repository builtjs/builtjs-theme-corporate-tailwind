...
import * as pageData from "../data/pages.json";
...
export const fetchCollections = async (section) => {
  ...
  for (const [contentTypeSlug, collection] of collections) {
    ...
    let { items } = await import(
      `../data/collections/${contentTypeSlug}.json`
    );
    ...
    if (collection.populate) {
      for (let i = 0; i < res.length; i++) {
        ...
        for (let j = 0; j < collection.populate.length; j++) {
          ...
          const contentTypeRes = alasql(`SELECT * FROM ?`, [contentTypeData.contentTypes]);
          const contentType = contentTypeRes[0] ? contentTypeRes[0] : null;
          if (!contentType) {
            return;
          }
...
export const fetchPage = async (pageSlug, type) => {
  ...
  let res = alasql(`SELECT * FROM ? WHERE slug = '${pageSlug}'`, [pageData.pages[type]]);
  if (res.length) {
...
export const fetchSection = async (sectionSlug) => {
  ...
  let res = alasql(`SELECT * FROM ? WHERE slug = ${sectionSlug}`, [sectionData.sections]);
  return res.length ? res[0] : null;
};
...
export const fetchTemplate = async (templateSlug) => {
  ...
  let res = alasql(`SELECT * FROM ? WHERE slug = ${templateSlug}`, [templateData.templates]);
  return res.length ? res[0] : null;
};
