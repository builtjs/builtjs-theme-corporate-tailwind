...
export const fetchItemById = async (contentTypeSlug, entryId) => {
  ...
  const collectionName = contentType.pluralDisplayName
    ...
    .toLowerCase();
  let itemData = await fetchData(`/data/collections/${collectionName}.json`);
  let res = alasql(`SELECT * FROM ? WHERE __id=${entryId}`, [itemData.items]);
  return res.length ? res[0] : null;
};
...
export const fetchCollections = async (section) => {
  ...
  // for (const [contentTypeSlug, collection] of collections) {
    ...
    newCollections[collection.slug] = newCollection;
  }
...
export const fetchPage = async (pageSlug, type) => {
  ...
  if (res.length) {
    let layoutData = await fetchData(`/data/templates/layout.json`);
